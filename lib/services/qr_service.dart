    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonData.containsKey('error') || 
            ...
            (jsonData['message'].toString().toLowerCase().contains('не найдена') ||
             jsonData['message'].toString().toLowerCase().contains('not found'))) {
        ...
        throw QRServiceException('Не удалось загрузить данные: ${response.statusCode}');
      }
      ...
      if (e.toString().contains('SocketException') || 
        ...
        throw QRServiceException('Не удалось подключиться к серверу. Проверьте, что backend запущен.');
      }
